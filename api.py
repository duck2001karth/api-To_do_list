...
def get_task_v2(id):
    this_task=[task for task in tasks if task['id'] == id]
    return jsonify({'tasks': this_task[0]}) if this_task else jsonify({'status': 'ID inexistente'})
...
if __name__ == '__main__':
    app.run(debug = True)    
    """
    Modifica este programa para añadir el campo timestamp (2 campos, creacion, actualizacion)
